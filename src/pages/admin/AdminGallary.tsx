...
const AdminGallery = () => {
  ...
  const [category, setCategory] = useState(""); // Now an open input string
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    if (!file || !title.trim() || !category.trim()) {
      return toast.error("File, Title, and Category are required");
    }
          ...
          <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 p-8 sticky top-0">
            ...
            <form onSubmit={handleUpload} className="space-y-5">
              ...
              >
                ...
                <input ref={fileInputRef} type="file" hidden onChange={handleFileChange} accept="image/*,video/*" />
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="relative">
                   <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300"><Hash size={16}/></span>
                   <input
                    type="text"
                    value={category}
                    onChange={(e) => setCategory(e.target.value)}
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-12 pr-4 py-3.5 text-sm focus:bg-white focus:ring-2 focus:ring-[#355E3B]/20 outline-none transition-all placeholder:text-slate-400"
                    placeholder="Category (e.g. Evidence, Event)"
                  />
                </div>
              ...
              <button
                type="submit"
                disabled={loading || !file || !title.trim() || !category.trim()}
                className="w-full bg-[#355E3B] text-white py-4 rounded-2xl font-black text-[11px] uppercase tracking-[0.2em] shadow-lg shadow-[#355E3B]/20 hover:bg-[#2a4b2f] hover:-translate-y-0.5 active:translate-y-0 transition-all disabled:opacity-50 disabled:translate-y-0"
              >
          ...
          {loading && items.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {filteredItems.map((item) => (
                <div key={item._id} className="group bg-white rounded-3xl overflow-hidden shadow-sm border border-slate-100 hover:shadow-xl transition-all duration-300">
                  <div className="relative h-48 bg-slate-900 overflow-hidden">
                    {item.resourceType === "video" ? (
                      <video className="absolute inset-0 w-full h-full object-cover opacity-60" src={item.url} muted />
                    ) : (
                      <img src={item.url} alt={item.title} className="w-full h-full object-contain group-hover:scale-105 transition-transform duration-500" />
                    )}

                    <div className="absolute top-4 left-4">
                      <span className={`text-[8px] font-black uppercase px-2.5 py-1 rounded-lg shadow-lg backdrop-blur-md ${
                        item.category === "evidence" ? "bg-red-500 text-white" : "bg-white/90 text-[#355E3B]"
                      }`}>
                        {item.category}
                      </span>
                    </div>
                    ...
                    <button
                      ...
                      className="absolute top-4 right-4 p-2.5 bg-white text-red-600 rounded-xl opacity-0 group-hover:opacity-100 transition-all shadow-xl hover:bg-red-600 hover:text-white"
                    >
                      <Trash2 size={16} />
                    </button>
                    ...
                    {item.resourceType === "video" && (
                      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                         <Film className="text-white/30" size={32} />
                      </div>
                    )}
                  </div>

                  <div className="p-5">
                    <h3 className="font-bold text-[#1A2F1F] text-sm truncate uppercase tracking-tight mb-3">{item.title}</h3>
                    <div className="flex items-center justify-between pt-3 border-t border-slate-50">
                      ...
                      <div className="flex flex-col text-right">
                        <span className="text-[8px] text-slate-300 font-black uppercase tracking-widest leading-none mb-1">Analyst</span>
