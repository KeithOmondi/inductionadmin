...
const JudgeDashboardPage = () => {
  ...
  const { guestList, loading: guestLoading } = useAppSelector(
    (state) => state.guest
  );
  const {
    unreadCount,
    chatMessages,
    loading: chatLoading,
  } = useAppSelector((state) => state.userChat);

  /* -------------------- COUNTDOWN LOGIC -------------------- */
  const eventDate = new Date("2026-03-14T00:00:00");
          ...
          <div className="lg:col-span-2 bg-white border border-slate-200 rounded-sm p-8 shadow-sm">
            ...
            {priorityEvent ? (
              <div className="flex flex-col md:flex-row gap-8 items-start mb-8">
                ...
                {/* Date Square */}
                <div className="bg-[#355E3B] p-4 text-center min-w-[100px] rounded-sm">
                  <p className="text-[#EFBF04] text-[10px] uppercase font-bold tracking-tighter">
                    {new Date(priorityEvent.date)
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-slate-500 text-sm max-w-md leading-relaxed">
                    {priorityEvent.description ||
                      "Please ensure all guest details are submitted for clearance."}
                  </p>
                </div>
              </div>
            ) : (
