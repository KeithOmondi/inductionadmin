...
const AdminHeader: React.FC = () => {
  ...
  // 1. Redux State - Now pointing to adminChat
  const { user } = useAppSelector((state) => state.auth);
  const { permission, loading: pushLoading } = useAppSelector((state) => state.push);
  const { unreadCount } = useAppSelector((state) => state.adminChat); // Corrected Selector
  ...
  const handleBellClick = () => {
    if (permission !== "granted") {
      ...
      // Clear unread count when the admin opens the notification panel
  ...
  return (
    <header className="h-20 bg-white border-b border-slate-200 px-8 flex items-center justify-between z-[50] sticky top-0">
      <div className="flex flex-col">
        ...
        <h2 className="text-xl font-serif font-bold text-slate-800">{getPageTitle()}</h2>
      </div>
        ...
        <div className="flex items-center gap-2 border-r border-slate-200 pr-6">
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            >
              ...
              {/* Dot only shows if there are actual unread messages */}
            ...
            {isNotifOpen && (
              <div className="absolute right-0 mt-4 w-72 bg-white border border-slate-200 rounded-xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2">
                <div className="p-3 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                  <span className="text-[10px] font-black uppercase tracking-tighter text-slate-500">Registry Notifications</span>
                ...
                <div className="max-h-60 overflow-y-auto p-4 text-center">
                  {unreadCount > 0 ? (
                    <p className="text-[11px] text-slate-600 font-medium">
                      You have {unreadCount} unread message{unreadCount > 1 ? 's' : ''} in the communication logs.
                    </p>
                  ) : (
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">No unread alerts</p>
                  )}
                </div>
              </div>
            )}
